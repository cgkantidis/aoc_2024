...
std::uint64_t
make_page_valid(std::vector<std::uint64_t> &page,
                std::map<std::uint64_t, std::set<uint64_t>> &order_rules) {
  std::uint64_t num_swaps{};
...
std::uint64_t
get_page_value(std::vector<std::uint64_t> &page,
               std::map<std::uint64_t, std::set<uint64_t>> &order_rules) {
  return make_page_valid(page, order_rules) == 0 ? 0 : page[page.size() / 2];
}
...
std::uint64_t
sum_middle_of_invalid(std::ranges::range auto &&lines) {
...
tests() {
  ...
  ASSERT(sum_middle_of_invalid(lines) == 123);
}
...
main(int argc, char const *const *argv) {
  ...
  fmt::println("{}", sum_middle_of_invalid(lines));
  return 0;
}
